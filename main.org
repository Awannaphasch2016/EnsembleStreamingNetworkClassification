...
Dynamic graph extends static graph to include time variables which added time dimension to the problem.


Additional degree of freedom includes network representation (lossy vs lossless),

*** TODO dynamic behavior on graph (communication behavior between nodes) and topological evolution (structure,features,role evolution of a graph). :actionable:

The temporal dimension can yield non-trivial temporal features such as interaction distribution, inter-event distribution (also known as burstiness), heterogenous activity distributions, causality constraints, evolution of topological pattern [[cite:&holme2012temporal;&holme2015modern]]. Furthermore, Holme et al. [[cite:&holme2012temporal]] discuss that temporal features such as burstiness depends on structure in which dynamic graph represented, such as different network, aggregated network, time-varying network (a graph where edges are labeled based on time interval where they present in the graph). For static network, degree distribution is established itself as one of the most fundamental statistic. However, the argument does not hold true even for the simplest form of temporal network [[cite:&holme2015modern]]. Simple temporal feature such as time of node and edges first appearance, time interval of node/edges presents from beginning to the end under some conditions are more important factor concerning nodes dynamic and evolution of graph structure.

*** TODO write about the following: :actionable:
...
dynamic graph models are classify by 3 aspects: node dynamic, temporal granularity.

*** impact of dynamic graph construction to node dynamic and graph evolution.
The topic of When graph construction is extremely important in data driven modeling such as deep learning. This is because deep learning models are directly trained on graph representation. However, this topic doesn't get as much attention as it should. Furthermore, according to Holme et al. [[cite:&holme2015modern]], problem of representation of graph has been solved yet. There is no way to include all relevant dimension of dynamic graph into one image. At the moment, which representation to choose depends on representation's accuracy and information to include or discard. That's it. One must first chooses either between lossy and lossless representation. Illustration are a great to support discussion, but, without specifying assumption of the representation clearly, motivation, problem, and message an author attempts to convey can be misleading. For more information please refer to Holme et al. [[cite:&holme2015modern]]

*** DONE what is burstiness property? you can give example
CLOSED: [2022-03-30 Wed 19:13]
:PROPERTIES:
:ID:       d6406960-48a8-406f-acea-40e322fe86ca
:END:
inter-event time distribution which is the frequency distribution between the events. If the events are independent and drawn from a uniform distribution, then the inter-event time distribution will be exponential. However, empirical data set usually has fat-tailed, scale-free rather than uniform distribution. Coefficient of variation, bustiness, is used to characterized scale-free degree inter-event distribution.

Using data-driven analysis, there are usually too few data point to accurately measure temporal structure accurately. Moreover, temporal structure, such as link burstiness, between node often has a fat-tailed distribution which is a problem when average over the value and most link have too little contacts to be good representation of burstiness. Hence, we want to emphasize that dataset that machine learning and deep learning models are trained on need to be improve to take in account of temporal pattern.
...
**** TODO I don't think "dynamic over graph" and "dynamic on graph" change this into something like "node dynamic", "graph evolution"  which depends on 3 factors:
