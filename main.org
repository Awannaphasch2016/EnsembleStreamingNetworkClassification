...
| ensemble parameters | $E_{i}$         | i-th classifier model in ensemble                            |
...
\begin{algorithm*}
\begin{algorithmic}[1]
\REQUIRE  $data$,$ws$,$bs$,$s$,$pw$
\ENSURE $score$
\STATE $edge\_index = ws$
\STATE $classifiers\_set = Empty$
\WHILE{$edge\_index < data.size$}
    \STATE $Ensembles = get\_all\_ensembles(edge\_index, ws)$
    \FOR{each $index$ in $ensembles$}
        \STATE $tepmoral\_parameters\_set = get\_tepmoral\_parameters(ws, index)$
        \FOR{$(pn, kl, s)$ in $temporal_\_parameters\_set$}
            \STATE $E = train\_classifier(pn, kl, s, bs, epoch)$
            \STATE add $E$ to $classifier\_set$
        \ENDFOR
    \ENDFOR
